...
严格遵循 [`doc/code-style.md`](doc/code-style.md)：4 空格、Allman、大写蛇形宏、小写蛇形函数，静态函数加 `_` 前缀；头文件 API/结构体写 Doxygen，并运行 AStyle。性能优先，再减少 ROM/RAM；文件长度仅作审查信号，低耦合、高内聚优先，不为缩短文件拆散共享状态；避免非必要防御，硬件/并发场景保留必要防护。有序操作使用单一 `exit`，以 `goto` 逆序清理；日志简短，多失败点可在出口统一记录行号。
