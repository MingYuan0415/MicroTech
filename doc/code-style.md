...
资源申请、初始化或多个有顺序的操作应使用单一出口，并在 `exit` 中按资源获取的逆序清理，避免在多个分支中重复释放资源或隐藏出口。函数包含多个需要区分和打印的失败分支时，可保存错误码和源码行号，仅在统一出口输出一次必要日志。`MT_ERROR_HANDLE` 定义在 `mt_log.h` 中，包含该头文件即可使用（`__LINE__` 用于记录触发错误的源码行）：
...
- 成功路径也必须经过 `exit` 的返回语句，函数保持单一返回点。没有资源需要清理时，不要为了形式强行使用 `goto`。
