...
`contracts/provisioning`（当前钉住 `85f82d4`，P0 draft freeze）；本仓库固件
`ble_runtime` 是参考实现，不替代契约。wire 尚未冻结，README 明确 draft 产物
"not evidence of Android interoperability"。

消费方必须：

- 以 Git submodule 钉住契约 commit（当前 `85f82d4`），不引用浮动的分支指针；
...
| `contracts/provisioning/docs/device-link-security-v1.md` | SC bonding、identity/LTK 校验要求 |
...
1. **固件安全 admission 为 fail-closed**：生产路径不设置 `SC_BOND_VERIFIED` 与
   `identity_known`（SMP bond-store identity/SC/LTK 校验未实现），session/control
   通道的 authorized 会话实际不可达。App 端可先实现并验证：广播发现、GATT 特征
   读写、MTU 协商与分片重组、`link_state` 读取与轮询、错误码路径；安全会话联调
   需等固件 SMP 校验落地（见实施文档遗留项）。
2. **事务串行化**：一次仅一个未确认响应；固件在最终 indication 确认前对后续请求
   返回 `BUSY`（LinkError=10）。客户端必须等待上一响应的最终片确认后再发下一请求。
3. **boot id 不匹配是终态**：客户端发现 `link_state.boot_id` 变化即视为设备重启，
   必须重建会话（重新发现/重连），不要继续旧会话。
4. **MTU 23 是强制基线**：分片重组必须以 MTU 23 为最坏情况实现；协商 MTU 后可
   放大帧长，但协议路径必须保持 MTU 23 正确。
5. **超时契约**：2 s 指示确认窗口、5 s 重组 idle 窗口由固件执行；客户端侧应有
   对应的请求超时与重试/放弃策略（具体时限以 framing 文档为准）。
6. **fake 授权在固件生产关闭**：`authorize` 相关请求在生产返回
   `UNSUPPORTED_OPERATION`，客户端应将其视为"未实现"，不进入依赖授权的流程。

## 4. 建议开发顺序

1. 集成契约 submodule + buf 生成管线；跑通 conformance golden/semantic 向量。
2. BLE 基础：扫描发现（device-link-discovery）、连接、MTU 协商、CCCD 订阅。
3. `link_state` 读取与事件订阅（先做无安全要求的公开通道）。
4. 帧收发：envelope 编码、分片提交、重组解码（以 MTU 23 为基线，对照 fixtures/
   framing 向量）。
5. 会话状态机与事务串行化（lifecycle + BUSY 处理）。
6. 错误码映射与超时/断连恢复（errors.md、framing 超时）。
7. 待固件 SMP/SC 校验落地后，联调 Security 2 会话与授权流程（security 文档）。
