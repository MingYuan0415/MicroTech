...
6. 待机准入技术债：`device_link_service_is_busy()` 在窗口或任意 ACL 存在时阻止
   待机（保守策略，避免休眠期间持活 ACL）。P3.3/P3.4 落地会话感知准入与断开
   路径后，空闲未授权 ACL 应静默退出待机准入；当前行为已记录为显式范围外。
7. 显示基准 BLE 门禁为 ACL 级：`ble_connected` 模式验证窗口打开与连接保持
   （无抖动），Security 2 受保护流量验收在 P3.4 回归后恢复计数门禁。
